...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/809809xyz/2ui/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
install_bbr() {
    bash <(curl -L -s https://raw.githubusercontent.com/809809xyz/2ui/master/bbr.sh)
    if [[ $? == 0 ]]; then
        echo ""
...
update_shell() {
    wget -O /usr/bin/v2-ui -N --no-check-certificate https://github.com/809809xyz/2ui/raw/master/v2-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
